...
class StudentEnrollment(models.Model):
    ...
    Tracks overall course progress and attendance.
    """
    ...
    completed_modules = fields.Integer(
    ...
    def create(self, vals_list):
        """Generate a unique enrollment number for each enrollment."""

        for vals in vals_list:
            if vals.get('name', _('New')) == _('New'):
                vals['name'] = (
                    self.env['ir.sequence'].next_by_code('student.enrollment')
                    or _('New')
                )
    ...
    @api.depends(
        ...
        'progress_ids.is_completed'
    )
    def _compute_progress(self):
        """Calculate total, completed and percentage of course modules."""
        ...
        for record in self:
            ...
            completed = len(
                record.progress_ids.filtered(
                    lambda progress: progress.is_completed
                )
            )
            ...
            if total > 0:
                ...
                # Automatically update enrollment status
                # according to course progress.
                if (
                    record.progress_percentage >= 100.0
                    and record.state in ('enrolled', 'in_progress')
                ):
                    record.state = 'completed'

                elif (
                    record.progress_percentage > 0.0
                    and record.state == 'enrolled'
                ):
            ...
            if total > 0:
                present_or_late = len(
                    record.attendance_ids.filtered(
                        lambda attendance: attendance.state in (
                            'present',
                            'late'
                        )
                    )
